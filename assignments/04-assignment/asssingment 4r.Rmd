...
*-note: i tried to work R in VScode for assignment 3, but the kernel kept dying and I spent a few hours trying to figure out how to run jupyter properly via anaconda prompt,before eventually giving up and submitting what I had, which is why it didn't run properly last time (cran mirrors and Ir kernel are surprisingly finicky with VS) Went simple this week with classic R studio, but now that we learned about Quarto in 350 - that may be the best option for future assignments!*
...
null hypothesis: ti=0 for all i alternative hypothesis: ti does not =0 for all i

Since a two sided alternative states that the alternative would not equal 0, it makes the most sense to use a two tailed test - because we also have to consider that it isn't just about the existence of weight loss, it can increase OR decrease weight loss!

# 5. Below is an R function to calculate statistical power in R. Assuming that \$\mu\_t = 55, $\mu_c = 50$, $\sigma = 30$, $n = 100$, and $\alpha = 0.05$, calculate the statistical power of the test. How many individuals would be necessary for the statistical power to be 0.8?
...
# The statistical power is 0.705, which means there is a 70.5% chance of detecting a true treatment effect
```
...
# the power is usually worth it if its around 0.8. With a sample size of 2400, the statistical power is 0.784, which is close enough for me to say  the study is likely worth conducting - but a slightly higher sample size would make it better
```
...
note: chatgpt was used to double check the next few problems since it kept returning a FUN error
...
A two-year term reduces bill introductions by 16.74 in texas when in Arkansas a two-year term reduces bill introductions by 10.09 The effect is only statistically significant in Arkansas
...
The average treatment effect (ATE) across both states is -13.22 bills which means overall there are less bills with these senators vs 4 year
...
Pooling assumes that treatment effects are identical across both states, but differences in legislative behavior/political culture in states could lead to heterogeneous treatment effects. If Arkansas has a generally lower number of bills introduced than Texas, the pooled estimate could be misleading.
...
The randomization inference test checks whether the observed effect could be due to random chance. f the p value is low, we can reject the sharp null hypothesisc - so the term length likely has an actual impact.
